 ...
 * }
 */
async function initResolve({ MAIN_FILE_URL, log = noop, }) {
    log('debug', `🛂 - Initializing the resolve service (resolving from "${MAIN_FILE_URL}")!`);
    /**
    ...
    return (path) => {
        ...
        const fqPath = import.meta.resolve(moduleIsRelative ? new URL(path, MAIN_FILE_URL).toString() : path);
